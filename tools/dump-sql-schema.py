...
def load_config(config_file):
    ...
    for field in required_fields:
        if field not in db_config:
            print(
                f"Error: Required field '{field}' not found in database configuration.",
                file=sys.stderr,
            )
            sys.exit(1)
...
def dump_schema(db_config, output_file=None, schema_name="papersorter"):
    ...
    cmd = [
        ...
        "--schema",
        schema_name,  # Only dump the papersorter schema
...
"""
        ...
        print(
            "Error: pg_dump command not found. Please ensure PostgreSQL client tools are installed.",
            file=sys.stderr,
        )
        sys.exit(1)
...
""",
    ...
    parser.add_argument(
        "-o", "--output", help="Output file for schema (default: stdout)"
    )
    ...
    parser.add_argument("--host", help="Database host (overrides config file)")

    parser.add_argument(
        "--port", type=int, help="Database port (overrides config file)"
    )

    parser.add_argument("--database", help="Database name (overrides config file)")

    parser.add_argument("--user", help="Database user (overrides config file)")

    parser.add_argument("--password", help="Database password (overrides config file)")
