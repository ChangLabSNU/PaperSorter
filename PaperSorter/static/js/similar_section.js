...
window.SimilarSection = (function() {
    ...
    fetch(`/api/feeds/${sourceFeedId}/similar`)
      ...
      .then(data => {
        ...
        if (similarArticles.length > 0) {
          ...
          // Enable summary section (admins only check can be added server-side later)
          const sum = document.getElementById('similarSummarySection');
  ...
  function generateSummary() {
    ...
    initial.style.display = 'none'; loading.style.display = 'block'; text.style.display = 'none'; text.innerHTML = '';
    fetch('/api/summarize', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ feed_ids: ids }) })
      .then(r => r.json()).then(data => {
        ...
        if (data.success) {
          text.innerHTML = `<div class="summary-text">${data.summary_html || data.summary_markdown || 'No summary available'}</div>
          ...
          text.innerHTML = '<div class="error">Failed to generate summary</div>';
        }
      }).catch(() => {
        loading.style.display = 'none'; text.style.display = 'block'; text.innerHTML = '<div class="error">Failed to generate summary</div>';
      });
  }
  ...
  function generatePoster() {
    ...
    const text = document.getElementById('similarSummaryText');
    const ids = [sourceFeedId, ...similarArticles.map(a => a.rowid)];
    initial.style.display = 'none'; loading.style.display = 'block'; content.style.display = 'none'; text.style.display = 'none';
        ...
        const poll = setInterval(() => {
          fetch(`/api/poster-status/${jobId}`).then(r => r.json()).then(s => {
            if (s.status === 'completed') {
              clearInterval(poll); loading.style.display = 'none';
              content.innerHTML = `
              ...
              clearInterval(poll); loading.style.display = 'none'; content.innerHTML = `<p style="color: var(--feedback-error-title-color);">Failed to generate poster${s.error ? `: ${s.error}` : ''}</p>`; content.style.display = 'block'; initial.style.display = 'flex';
            }
          }).catch(() => { clearInterval(poll); loading.style.display = 'none'; content.innerHTML = `<p style="color: var(--feedback-error-title-color);">An error occurred while checking job status.</p>`; content.style.display = 'block'; initial.style.display = 'flex'; });
        }, 3000);
        setTimeout(() => { clearInterval(poll); if (loading.style.display !== 'none') { loading.style.display = 'none'; content.innerHTML = `<p style=\"color: var(--feedback-error-title-color);\">Generation timed out. Please try again.</p>`; content.style.display = 'block'; initial.style.display = 'flex'; } }, 300000);
      }).catch(() => { loading.style.display = 'none'; content.innerHTML = `<p style="color: var(--feedback-error-title-color);">An error occurred while generating the poster.</p>`; content.style.display = 'block'; initial.style.display = 'flex'; });
  }
